...
static esp_err_t appSpecificUriHandler(httpd_req_t *req) {
    // Your handling code here, for example:
    httpd_resp_send(req, "Custom URI Handler Response", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
void startWebServer() {
  ...
  if (res == ESP_OK) {
    ...
    // Conditionally register the custom URI handler
    #ifdef CUSTOM_URI
    httpd_uri_t customUri = {
        .uri = CUSTOM_URI, // The URI to handle, defined by the preprocessor directive
        .method = HTTP_POST, // HTTP method
        .handler = appSpecificUriHandler, // Handler function
        .user_ctx = NULL // User context (if any)
    };
